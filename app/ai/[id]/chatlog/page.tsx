...
const Chatlog = () => {
  ...
  return (
    <>
      ...
      <div className="w-full h-full flex text-black gap-2 px-4 overflow-y-auto">
        <div className="flex-items-col h-fit w-[40%] border-r-gray-200 border-r gap-2 pb-[40px]">
          {chatData.map(({ message, time }, index) => (
            <div key={index} onClick={() => setSelectedIndex(index)}
              className={`cursor-pointer ${index === selectedIndex ? 'bg-gray-200 justify-items w-[90%] h-[60px] b px-3 rounded-[2px] shadow-19' : 'justify-items w-[90%] h-[60px] bg-gray-50 hover:bg-gray-100 px-3 rounded-[2px] shadow-19'}`}
            >
